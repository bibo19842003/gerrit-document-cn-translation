...
根据 `refs/meta/config` 缓存 project 的描述信息。`projects` 缓存的 persisted variant，意味着不占内存空间。
...
如果配置多个服务器信息的话，服务器的信息需要用空格分开，如：`server = ldaps://ldap1 ldaps://ldap2`。相关信息可参考 [issue 10841](https://bugs.chromium.org/p/gerrit/issues/detail?id=10841)。
...
如果设置为 `ALLOW`，只有匹配到的邮件（`receiveemail.filter.patterns`）才会进行处理。

如果设置为 `BLOCK`，只有未匹配到的邮件（`receiveemail.filter.patterns`）才会进行处理。

默认值：`OFF`

之前的 'BLACKLIST' 和 'WHITELIST' 参数值已废弃，在未来的版本中，此两个参数值会被移除。
...
如果设置了参数值，那么会被添加到 allow 列表中，这样 gerrit 会按照对应列表发送邮件。如果设置的是邮箱地址，那么邮箱会被添加到列表中；如果设置的是 domian 信息，那么此 domain 中的邮箱被被添加到 allow 列表。
...
`service users` 使用 SSH 线程的最大数量。如果设置为 0，那么 `service users` 与 `interactive` 的请求在同一个队列中处理。其他的值，会从 `interactive` 线程中分配出对应的固定的线程给 `service users`。

如果设置的 `service users` 线程数量大于总的 `sshd.threads` 线程数，那么需要上调 `sshd.threads` 的参数值。
...
设置停止时间。在 `SSH daemon` 关闭前，使 open 状态的 SSH session 再持续一段时间，这段期间内，不会接受新的请求。此参数被用来执行滚动性重启操作。
...
Gerrit gc 使用场景：[gerrit gc](cmd-gc.md) 命令和 Gerrit 配置的计划任务 `gc.startTime` 与 `gc.interval`。

**gc.auto**

当松散对象的数量大于此参数值的时候，gerrit 会自动执行 gc。默认值为 6700。

设置为 0 意味着不自动执行 gc，即使其他类似的配置配了有效值，也不会生效，如：`gc.autoPackLimit`。
...
当未被标识为 `*.keep` 的 pack 文件数量超过此参数值时，会自动执行 gc 操作。默认值为 50。若设置为 0 意味着取消此操作。`gc.auto` 设置为 0 同样会取消此操作。

**gc.packRefs**

gc 时是否执行 `git pack-refs`
...
默认为 90 days。参数值 "now" 清理所有过期信息；"never" 为不清理。

**gc.reflogExpireUnreachable**

移除过期的 reflog 和不可访问的信息。

默认为 30 days。参数值 "now" 清理所有过期信息；"never" 为不清理。
...
支持的版本：
* 0: 原始的 wire protocol
* 1: 原始的 wire protocol 附加版本信息
* 2: wire protocol 版本 2。加快客户端 fetch refs 的速度。

### Section receive

**receive.autogc**

Gerrit 3.2 ，`git-receive-pack` 接收到数据后会默认执行 gc 操作。参数值设置为 `false` 意味着取消此操作。推荐设置为 `false`，这样可以减小系统的负载。
不建议使用 `gc.startTime` 和 `gc.interval`，建议优先使用 cron 来执行 gerrit gc 操作。
从 Gerrit 3.3 开始，gerrit 初始化时如果选择非手动配置，那么 `etc/jgit.config` 文件中自动配置 `git-receive-pack = false`；如果选择手动配置，那么会有相应的提示信息。
